...
use crate::{aes::aes::aes, args_handling::args_struct::{get_crypto_system, ArgsStruct}, rsa::rsa::rsa};

pub fn pgp_aes(args: ArgsStruct) -> Option<String> {
    let tmp = args.symmetric_key.unwrap_or(String::new());
    let keys: Vec<&str> = tmp.split(":").collect();
    if keys.len() != 2 {
        eprintln!("pgp-aes requires a symmetric key and a RSA key formatted like so: \"sym_key:rsa_key\"");
        return None;
    }
    if !args.cipherd_to_decipherd {
        ...
        res += "\n";
        res += aes(ArgsStruct { crypto_system: get_crypto_system("aes")?, cipherd_to_decipherd: args.cipherd_to_decipherd, asymmetric_key: None, symmetric_key: Some(keys[0].to_string()), stream_mode: args.stream_mode, message: args.message }, true)?.as_str();
        Some(res)
    } else {
        ...
        aes(ArgsStruct { crypto_system: get_crypto_system("aes")?, cipherd_to_decipherd: args.cipherd_to_decipherd, asymmetric_key: None, symmetric_key: Some(hex::encode(cipher_key)), stream_mode: args.stream_mode, message: args.message }, true)
    }
}
